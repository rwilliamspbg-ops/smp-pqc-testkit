//! Constant-time check for ML-KEM-1024 decapsulation, using the [DudeCT]
...
//! Run with:
//! ```bash
//! cargo run -p smp-pqc-core --release --example dudect_ml_kem_1024_decap
//! ```
...
fn ml_kem_1024_decapsulate_valid_vs_corrupted_ciphertext(
    runner: &mut CtRunner,
    rng: &mut BenchRng,
) {
...
ctbench_main!(ml_kem_1024_decapsulate_valid_vs_corrupted_ciphertext);
